...
async def validation_exception_handler(request, exc: RequestValidationError):
    ...
    if path == "/api/process_member_documents":
        # Mirror the HTTPException 422 handler, but for validation errors
        return JSONResponse(status_code=200, content={
            'success': True,
...
async def process_member_documents(
    ...
    try:
        ...
        logger.info("Processing member documents...")
        debug_flag = False
        try:
        ...
        logger.error(f"Member documents processing error: {str(e)}")
