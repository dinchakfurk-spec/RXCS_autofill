  ...
  "member_genders": []
    ...
    def process_member_documents(self, member_index: int, aadhaar_files: list = None, pan_files: list = None, salary_files: list = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Prepare content with all images
            # Determine which document types are present for better context
            doc_types_present = []
            if aadhaar_files:
                doc_types_present.append("Aadhaar card")
            if pan_files:
                doc_types_present.append("PAN card")
            if salary_files:
                doc_types_present.append("Salary slip")
            
            doc_types_text = ", ".join(doc_types_present) if doc_types_present else "provided documents"
            
            content = [
                {
                    "type": "text",
                    "text": f"""Extract member information from the {doc_types_text} provided. You may receive one or more document types (Aadhaar card, PAN card, and/or Salary slip). Extract all available information from whatever documents are present.

IMPORTANT: Extract information from whatever documents are provided. Not all fields will be available in every document type:
- Aadhaar card typically contains: Full Name, Residential Address, Pincode, State, City, Country, Gender, Aadhaar Number
- PAN card typically contains: Full Name, Father's Name, PAN Number
- Salary slip typically contains: Full Name, Company/Firm Name, Salary/Business Income, Years of Experience, Occupation
...
Return JSON format with all fields (use empty strings for fields not found):
{{
...
Extract and return whatever information is visible in the provided documents. Do not require all document types to be present.

For Aadhaar card specifically:
- Carefully read the address block (usually near the bottom, often after the word 'Address', 'To', or 'C/O').
- If any address text is visible on the Aadhaar card, do NOT leave residential_address, pincode, state, city or country empty.
- For pincode, extract the 6-digit postal code from the Aadhaar address.
- Always read and return the 12-digit Aadhaar number (remove spaces) if it is visible on the Aadhaar card image.""" 
                }
            ]
            ...
            logger.info(f"Sending member documents to GPT-4o Vision... ({len(all_base64_images)} pages from {doc_types_text})")
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You extract structured data from documents. Extract all available information from whatever documents are provided, even if only one document type is present. Return only valid JSON with empty strings for fields not found."},
                    {"role": "user", "content": content}
                ],
                temperature=0.1,
                max_tokens=2000
