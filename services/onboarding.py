...
class OCRProcessorService:
    ...
    def _convert_file_to_base64_images(self, file_path: str) -> List[str]:
        ...
        try:
            if file_extension == '.pdf':
                # Allow tuning via env; default processes all available pages
                max_pages_env = os.getenv("RXCS_MAX_PAGES_PER_DOC")
                dpi_env = os.getenv("RXCS_PDF_DPI")
                max_pages = None
                if max_pages_env:
                    try:
                        parsed = int(max_pages_env)
                        if parsed > 0:
                            max_pages = parsed
                    except ValueError:
                        max_pages = None
                ...
                logger.info(
                    ...
                    f"(page_limit={'all' if max_pages is None else max_pages}, dpi={pdf_dpi})"
                )
                ...
                pages_to_render = page_count if max_pages is None else min(page_count, max_pages)
    ...
    def base_hospital_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "user_email": "",
            "user_password": "",
            "subvention_for_reimbursement": "0",
            "processing_fee_reimbursement": "0",
            "cashless_desk_subvention": "0",
            "subvention_no_cost_emi": "0",
            "processing_fee_cashless": "0",
            "processing_fee_nocost_emi": "0",
            "onetime_verification_fee": "0",
            "amount_collection_from_patient": "0",
            "fixed_rate": "0",
            "start_date": "",
            "end_date": "",
            "hospital_rates": []
    ...
    def _normalize_numeric_field(self, value: Any) -> str:
        """Extract first numeric token and return it as plain digits (default '0')."""
        if value is None:
            return "0"
...
CRITICAL FIELDS (must be filled whenever the document hints at them, even if wording differs):
- "subvention_for_reimbursement": any subvention/subsidy/discount tied to reimbursement cases.
- "processing_fee_reimbursement": processing fee/charge for reimbursement flows.
- "cashless_desk_subvention": subvention amount/percentage for the cashless desk/cashless cases.
- "subvention_no_cost_emi": subvention for no-cost EMI programs.
- "processing_fee_cashless": processing fee for cashless cases (per patient or %, capture verbatim).
- "processing_fee_nocost_emi": processing fee/charge for no-cost EMI.
- "onetime_verification_fee": any single/registration/verification/setup fee charged to the hospital.
- "amount_collection_from_patient": amount the partner collects upfront from patient (if any).
- "fixed_rate": any fixed slab/rate agreed between parties.

Synonyms may include phrases like "desk charges", "processing charge", "PF", "service fee", "discount", "subsidy", "support", "float", "% per case", "per patient fee", "no cost EMI support", etc. Map any such phrase to the relevant key.

For ALL of the above critical fields, return ONLY the numeric value (digits with optional decimal point). Remove units/words such as "INR", "Rs", "per case", "%", etc. Examples:
- "70% of the claim" → "70"
- "INR 599 per case" → "599"
- "0.5% processing fee" → "0.5"
If the value is stated as a range/slab, take the numeric part most representative of the hospital’s cost (e.g., "2% to 3%" → "2-3" is NOT allowed; pick the concrete number if available, otherwise leave "0"). If the document truly omits the value, return "0".

If the document expresses these values as percentages, per-patient fees, slabs, etc., copy the exact text (e.g., "2% per case", "INR 500 per transaction"). If a field truly is not mentioned, leave it as "".

Rules:
- If a value is clearly present, populate it verbatim (keep currency symbols/percent signs as seen).
...
"""
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert document-understanding engine. Think carefully, cross-check every page, and return only valid JSON matching the requested schema. Never emit commentary."},
                    {"role": "user", "content": content}
                ],
                temperature=0.0,
                max_tokens=2000
            ...
            for field in numeric_fields:
                schema[field] = self._normalize_numeric_field(schema.get(field))
